export function getCurrentYYYYMMDDHHmms() {
    const date = new Date();
    return `${date.getUTCFullYear()}-${padNumber(date.getUTCMonth() + 1, 2)}-${padNumber(date.getUTCDate(), 2)}t${padNumber(date.getUTCHours(), 2)}-${padNumber(date.getUTCMinutes(), 2)}-${padNumber(date.getUTCSeconds(), 2)}`;
}
