...
class BaseClient {
    ...
    async invokeScript(script, options) {
        const scriptInvocation = ProtobufMessage_1.command_request.ScriptInvocation.create({
            hash: script.getHash(),
            keys: options?.keys?.map((key) => typeof key === "string" ? Buffer.from(key) : key),
    ...
    async updateConnectionPassword(password, immediateAuth = false) {
        ...
        const response = await this.createUpdateConnectionPasswordPromise(updateConnectionPassword);
