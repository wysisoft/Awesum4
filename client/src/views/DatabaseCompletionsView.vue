...
export default {
  ...
  setup() {
    ...
    const completionTableOptions = {
      ...
      order: [[1, 'desc'], [2, 'desc']] // Sort by completion level first, then by completion date
    } as Config | undefined;
  ...
  methods: {
    async loadCompletionData() {
      ...
      var followerDatabases = await this.$awesum.AwesumDexieDB.serverFollowerDatabases
        .filter((x) => x.databaseId == this.$awesum.currentDatabase.id).toArray();

      // Get the total number of units in this database
      var totalUnits = await this.$awesum.AwesumDexieDB.serverDatabaseUnits
        .where('databaseId').equals(this.$awesum.currentDatabase.id).count();
      ...
      var databaseCompletionRecords = await this.$awesum.AwesumDexieDB.serverFollowerDatabaseCompletions
        .where('itemId').equals(this.$awesum.currentDatabase.id).toArray();
      ...
      var unitCompletionRecords = await this.$awesum.AwesumDexieDB.serverFollowerDatabaseCompletions
        .where('parentItemId').equals(this.$awesum.currentDatabase.id).toArray();
...
<template>
  <div class="pageView" :style="{backgroundImage: background}">
    <h1>{{ $awesum.currentDatabase.name }} {{ $t($resources.Completions.key) }}</h1>
    <div class="content" >
      ...
      <div v-if="completionData.length == 0">
        <div style="margin-left:2svmin;">No followers found for this database. When followers are added and complete assignments, they will appear here.
        </div>
      </div>
    </div>
  </div>
